(function ($) {
    ...
    $(document).ready(function () {
        // Failsafe: Only execute if search elements exist on the DOM
        if ($('.influencer-search-main').length === 0 && !ajax_vars.search_results_page_id) {
            return;
        }
        ...
        if (typeof ajax_vars !== 'undefined' && ajax_vars.search_results_page_id == ajax_vars.page_id) {
    ...
    // -------------------------------------------------------------------
    // PASTE YOUR SEARCH-SPECIFIC FUNCTION DEFINITIONS BELOW THIS LINE
    // -------------------------------------------------------------------
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    if (response.data.debug) {
                        render_brief_search_debug(response.data.debug);
                    }
                    ...
                    button.hide();
                    if (debug) {
                        render_brief_search_debug(debug);
                    }
                }
