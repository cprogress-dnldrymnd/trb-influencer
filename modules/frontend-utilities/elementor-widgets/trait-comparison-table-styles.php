...
/**
 * Style-tab controls shared by the two widgets that render the same grid — Widget_Feature_Comparison_Table
 * (every column, static CTAs) and Widget_Pricing_Table (same grid in "pricing mode", membership-aware
 * buttons, some columns hidden). Both target the same .dd-fc-* markup emitted by
 * DD_Feature_Comparison_Table::render_table(), so their look must stay stylable from one place.
 *
 * The control NAMES here are load-bearing: renaming any would orphan settings already saved on
 * existing Comparison Pricing Table widgets in the database.
 */
trait DD_Comparison_Table_Style_Controls {

    /**
     * Registers every shared Style-tab control. The caller owns the surrounding
     * start_controls_section()/end_controls_section() pair, so a widget can append its own
     * controls into the same section afterwards.
     *
     * @return void
     */
