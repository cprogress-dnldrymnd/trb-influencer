...
/**
 * Intercepts and filters the PM Pro page URL generation.
 * * This function specifically targets the 'checkout' page type. If the request originates 
 * from the custom 'sign-up' page, it modifies the form action URL to post back to itself. 
 * This maintains the operational state, ensuring that standard PM Pro validation errors 
 * are surfaced on the original view rather than redirecting to the default checkout slug.
 *
 * @param string $url  The fully qualified PM Pro URL generated by the core plugin.
 * @param string $page The specific PM Pro page context being requested (e.g., 'checkout', 'levels', 'billing').
 * * @return string The filtered URL. Returns the current page permalink if conditions are met, otherwise defaults to standard behavior.
 */
function dd_pmpro_retain_signup_state( $url, $page ) {
	// Isolate execution strictly to the checkout form URL generation.
	if ( 'checkout' === $page ) {
		// Identify if the DOM context is currently resolving the custom 'sign-up' route.
		if ( is_page( 'sign-up' ) ) {
			// Rewrite the destination payload to the active page, enforcing local form submission.
...
add_filter( 'pmpro_url', 'dd_pmpro_retain_signup_state', 10, 2 );