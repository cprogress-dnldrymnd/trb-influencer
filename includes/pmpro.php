 ...
 * * UPDATED: Includes dynamic parsing for prorated checkout totals and extracts exact future
 * start dates for the timeline UI.
 *
 * @return void
 */
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var pollDOM = setInterval(function() {
                ...
                if ($radios.length > 0 || $levelCost.length > 0) {
                    ...
                    // ==========================================
                    // DATE EXTRACTION LOGIC (UPDATED)
                    // ==========================================
                    // Scan the PMPro string for explicit future dates injected by the prorating engine
                    var startingMatch = sourceText.match(/starting\s+([^.]+)/i);
                    var billedMatch = sourceText.match(/billed(?:.*?)on\s+([a-zA-Z]+\s+\d{1,2}(?:st|nd|rd|th)?,?\s+\d{4})/i);

                    // If PMPro returns "...starting May 20, 2026." we capture it
                    if (startingMatch && startingMatch[1] && !startingMatch[1].toLowerCase().match(/^now$/)) {
                        startDateStr = startingMatch[1].trim();
                    } 
                    // Fallback for "...billed on May 20, 2026."
                    else if (billedMatch && billedMatch[1]) {
                        startDateStr = billedMatch[1].trim();
                    } 
                    // Fallback for standard Free Trials
                    else if (trialDays > 0) {
