 ...
 * Description: Uses MutationObserver to forcefully rename the checkout button on the membership checkout page. Integrates with PMPro Proration to enforce native delayed downgrades using strict structural mapping.
...
{
    // Target the specific checkout page URL provided
    if (! is_page(1551)) {
        return;
    }
?>
 ...
 * the current user's authentication status and membership tier.
 *
...
function dd_force_free_members_to_upgrade()
{
    ...
    // Intercept Free Level (15) Confirmation Page
    ...
    // Explicitly exempt core PMPro functional pages
...
function dd_influencer_style_pmpro_checkout()
{
    ...
    $real_plan_name = 'Membership Plan';
    $plan_description = '';
...
/**
 * Enforces the native PMPro Proration "Delayed Downgrade" Engine
 * -------------------------------------------------------------------------
 * The Proration Add-on relies on strict cost-per-day calculations which can 
 * fail to identify frequency shifts (e.g., Annual to Monthly) as downgrades.
 * This filter hardcodes your hierarchy mapping to guarantee the delayed demotion triggers natively.
 *
 * @param bool   $is_downgrade Current boolean evaluation by the Proration Add-on.
 * @param object $old_level    The user's current membership level object.
 * @param object $new_level    The membership level object being purchased.
 * @return bool                True if it's a downgrade, False otherwise.
 */
function dd_pmpro_force_native_delayed_downgrade($is_downgrade, $old_level, $new_level)
{
    // CONFIGURATION: Define your membership level hierarchy from LOWEST to HIGHEST tier.
    // ** IMPORTANT: Replace these integers with your actual PMPro Level IDs. **
    $level_hierarchy = array(
        15, // Rank 0: Lowest Tier (e.g., Free)
        1,  // Rank 1: Mid Tier (e.g., Essential Monthly)
        2,  // Rank 2: High Tier (e.g., Essential Annual)
        3   // Rank 3: Highest Tier
    );

    $old_rank = array_search($old_level->id, $level_hierarchy);
    $new_rank = array_search($new_level->id, $level_hierarchy);

    // Strictly enforce downgrade status if both levels are successfully mapped
    if ($old_rank !== false && $new_rank !== false) {
        // If the old rank is structurally higher than the new rank, explicitly flag as a downgrade.
        return ($old_rank > $new_rank);
    }

    // Fallback to the Proration Add-on's default evaluation if levels are unmapped
    return $is_downgrade;
}
add_filter('pmpro_is_downgrade', 'dd_pmpro_force_native_delayed_downgrade', 20, 3);