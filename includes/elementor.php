...
/**
 * Registers the custom myCRED visibility control within the Elementor editor.
 *
...
function dd_register_mycred_visibility_controls( $element, $args ) {
    ...
    $element->add_control(
        ...
        [
            ...
            'description' => __( 'Select when this element should be visible based on the myCRED balance.', 'dd-elementor-mycred' ),
        ]
    );
...
/**
 * Evaluates the myCRED visibility condition strictly for the frontend.
 *
...
function dd_evaluate_mycred_condition( $should_render, $element ) {
    ...
    // STRICT BYPASS: Ensure elements are ALWAYS visible when editing or previewing
    ...
    if ( ! empty( $rule ) ) {
        ...
        $balance = 0; // Default to 0 for guests

        // Fetch balance directly from the database
        if ( $user_id ) {
        ...
        if ( 'zero' === $rule && $balance > 0 ) {
            return false; // Hide: Rule requires 0, but user has points
        }

        if ( 'has_points' === $rule && $balance < 1 ) {
            return false; // Hide: Rule requires 1+, but user has 0
        }
    }
