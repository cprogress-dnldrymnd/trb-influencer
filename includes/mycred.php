 ...
 * the payment info, injects the user avatar, and restyles the Stripe submit button.
 *
 * @return void
 */
...
?>
    <style>
        /* Influencer-style CSS Overrides specifically mapped for myCred */
        ...
        .infl-plan-info h4, .infl-price-info h4 {
            margin: 0 0 2px 0 !important;
            font-size: 16px !important;
            font-weight: 700 !important;
        }

        .infl-plan-info span, .infl-price-info span {
            font-size: 14px;
            color: #b3b3b3;
        }
        ...
        .infl-timeline::before {
            ...
            left: 19px;
            top: 10px;
            bottom: 10px; /* Shortened since there's only 1 dot for one-time purchases */
            width: 1px;
            background: #000;
        }
        ...
        /* myCred Stripe Button Override */
        #mycred-stripe-payment-form button[type="submit"],
        .mycred-buy-wrapper button[type="submit"],
        .infl-pay-btn {
        ...
        #mycred-stripe-payment-form button[type="submit"]:hover,
        .mycred-buy-wrapper button[type="submit"]:hover,
        .infl-pay-btn:hover {
            background-color: #1fdf64 !important;
            transform: scale(1.02);
        }

        /* Clean up myCred's default payment box */
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Wait briefly to ensure myCred/Stripe DOM elements are fully loaded
            setTimeout(function() {
                
                // 1. Strict Guard: Only run if myCred Checkout elements are detected
                var $wrapper = $('.mycred-buy-cred-wrapper').first();
                if ($wrapper.length === 0) $wrapper = $('#mycred-stripe-payment-form').parent();
                if ($wrapper.length === 0) $wrapper = $('a:contains("cancel purchase")').parent();
                
                if ($wrapper.length === 0) return; // Exit if not a myCred checkout page

                // Wrap content in our max-width container to control layout
                $wrapper.wrapInner('<div class="dd-mycred-checkout-container"></div>');
                var $container = $('.dd-mycred-checkout-container');

                var avatarHtml = <?php echo wp_json_encode($avatar_html); ?>;

                // 2. Inject Header and Title Row
                ...
                '<h2>Checkout</h2>' +
                '<a href="javascript:history.back()">Change amount</a>' +
                '</div>';
            
            $container.prepend(headerHtml);

            // 3. Parse Data and Hide the myCred Information Table
            var creditsAmount = "0";
            var costAmount = "$0.00";

            var $table = $container.find('table').filter(function() {
                return $(this).text().indexOf('Credits') !== -1 && $(this).text().indexOf('Cost') !== -1;
            });

            if ($table.length > 0) {
                // Extract dynamic values from the table rows
                $table.find('tr').each(function() {
                    var rowText = $(this).text();
                    if (rowText.indexOf('Credits') !== -1) {
                        creditsAmount = $(this).find('td').last().text().trim() || $(this).children().last().text().trim();
                    }
                    if (rowText.indexOf('Cost') !== -1) {
                        costAmount = $(this).find('td').last().text().trim() || $(this).children().last().text().trim();
                    }
                });

                // Hide the table and the "Test Mode" header safely
                $table.hide();
                $table.prevAll().filter(function() {
                    var txt = $(this).text().trim();
                    return txt.indexOf('Test Mode') !== -1 || txt.indexOf('Testing') !== -1 || $(this).prop('tagName') === 'H3';
                }).hide();

            } else {
                // Fallback parsing just in case myCred changes its DOM structure
                var bodyText = $container.text();
                var credMatch = bodyText.match(/Credits\s+(\d+)/i);
                if (credMatch) creditsAmount = credMatch[1];
                
                var costMatch = bodyText.match(/Cost\s+([0-9.,]+\s*[A-Z]*)/i);
                if (costMatch) costAmount = costMatch[1];
            }

            // 4. Build myCred Summary HTML (Modified for One-Time Purchase structure)
            var influencerHtml = `
            ...
            // 5. Inject Summary Block ABOVE the payment form
            var $paymentFields = $('#mycred-stripe-payment-form, form.mycred-stripe-form');
            if ($paymentFields.length === 0) {
                // Fallback: look for the element containing "PAYMENT INFORMATION"
                $paymentFields = $container.find('div').filter(function() {
                    return $(this).text().trim() === 'PAYMENT INFORMATION';
                }).parent();
            }
            
            if ($paymentFields.length > 0) {
                $paymentFields.before(influencerHtml);
            } else {
                $container.append(influencerHtml); // Absolute fallback
            }

        }, 150); // Delay ensures myCred elements are fully parsed
    });
    </script>
<?php
}
